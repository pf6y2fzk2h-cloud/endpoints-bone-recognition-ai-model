...
"""
Evaluate the cascade model on labelled images (model_11_6kp).
=============================================================
Runs on all labelled images and reports the pixel distance between
predicted and ground-truth keypoints. Saves results to a Word document.
...
CONFIG = {
    ...
    'crop_size':        256,
    'seg_threshold':    0.2,
    'kp_threshold':     0.0,    # show all keypoints regardless of confidence
    'pad_fraction':     0.10,
    'num_images':       None,   # None = all labelled images
}
...
# =============================================================================
# KEYPOINT EXTRACTION (argmax — model_11 style)
# =============================================================================
...
def extract_keypoints(heatmap_tensor, threshold=0.0):
    """Argmax-based extraction — all keypoints returned regardless of confidence."""
...
def main():
    ...
    if not candidates:
        print("No labelled images found."); return

    n = CONFIG['num_images']
    selected = candidates if n is None else candidates[:n]
    print(f"Evaluating on {len(selected)} labelled images.\n")

    all_errors = []
    per_label_errors = {lbl: [] for lbl in ALL_KP_LABELS}

    doc = Document()
    doc.add_heading('Keypoint Evaluation Report — model_11_6kp', 0)
    doc.add_paragraph(
    ...
    for img_data in selected:
        ...
        orig_image = Image.open(os.path.join(CONFIG['images_dir'], img_name)).convert('L')
        orig_arr   = np.array(orig_image, dtype=np.uint8)
        orig_size  = orig_image.size

        gt_points = {p['label']: (p['x'], p['y']) for p in img_data.get('points', [])}
        ...
        doc.add_heading(img_name, level=1)
        doc.add_paragraph(
        ...
        vis_base = os.path.join(CONFIG['output_folder'], f'{Path(img_name).stem}_eval.png')
    ...
    if all_errors:
        for line in [
            ...
            f"Min / Max                : {np.min(all_errors):.2f} / {np.max(all_errors):.2f} px",
        ]:
        ...
        for cell, text in zip(tbl2.rows[0].cells, ['Keypoint','Mean error (px)','N images']):
            cell.text = text
            cell.paragraphs[0].runs[0].bold = True
            cell.paragraphs[0].runs[0].font.size = Pt(9)
    ...
    docx_path = os.path.join(CONFIG['output_folder'], 'evaluation_results.docx')
    doc.save(docx_path)

    print("\n" + "=" * 50)
    if all_errors:
        print(f"OVERALL RESULTS  ({len(selected)} images, {len(all_errors)} keypoints)")
